...
If the configurations were defined elsewhere, you could only reference them either by first creating direct reference via `configurations`  — as opposed to `configurations.create()` — or by using string literals within the `dependencies {}` block.
