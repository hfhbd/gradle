...
In Kotlin DSL, you can access and create extra properties using the `extra` accessor as a map as demonstrated in the following sample:
...
<2> Binds an existing extra property from the current context (the project in this case) to a `myProperty` reference
