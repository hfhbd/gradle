...
====
include::sample[dir="snippets/dependencyManagement/catalogs-settings/common",files="gradle/libs.versions.toml[]"]
====
...
Catalogs use a <<#sec::toml-dependencies-format,TOML format>> in which accessors map directly to the aliases and versions defined in it, providing type-safe access to dependencies and plugins.
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-classifiers/common/gradle",files="libs.versions.toml[tags=catalog]"]
====
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-classifiers/kotlin",files="build.gradle.kts[tags=use_classifier]"]
include::sample[dir="snippets/dependencyManagement/catalogs-classifiers/groovy",files="build.gradle[tags=use_classifier]"]
====
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-classifiers/kotlin",files="build.gradle.kts[tags=use_artifact_type]"]
include::sample[dir="snippets/dependencyManagement/catalogs-classifiers/groovy",files="build.gradle[tags=use_artifact_type]"]
====
...
Gradle downloads the <<#sec::toml-dependencies-format,TOML>> file from whatever repositories are configured in the `dependencyResolutionManagement` block and makes its entries available as type-safe accessors, just like a local `libs.versions.toml` file.
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-multipleCatalogs/kotlin",files="settings.gradle.kts[tags=multiple-catalogs]"]
include::sample[dir="snippets/dependencyManagement/catalogs-multipleCatalogs/groovy",files="settings.gradle[tags=multiple-catalogs]"]
====

Each catalog generates its own extension, so you reference them by name in build scripts:

====
include::sample[dir="snippets/dependencyManagement/catalogs-multipleCatalogs/kotlin",files="build.gradle.kts[tags=use-multiple-catalogs]"]
include::sample[dir="snippets/dependencyManagement/catalogs-multipleCatalogs/groovy",files="build.gradle[tags=use-multiple-catalogs]"]
====
...
The `version-catalog` plugin lets you declare a catalog and publish it as a Maven or Ivy artifact.
...
Then, use the link:{javadocPath}/org/gradle/api/plugins/catalog/CatalogPluginExtension.html[catalog extension] to declare the catalog entries:
...
Finally, apply either the `maven-publish` or `ivy-publish` plugin and configure a publication using the `versionCatalog` component:
...
When you run `./gradlew publish`, a `libs.versions.toml` file is generated from the catalog declaration and uploaded as a Maven artifact (with the `.toml` extension).
...
.gradle/distribution.versions.toml
====
include::sample[dir="snippets/dependencyManagement/catalogs-duplicateAndVerify/kotlin/gradle",files="distribution.versions.toml[tags=versions]"]
====

.gradle/test.versions.toml
====
include::sample[dir="snippets/dependencyManagement/catalogs-duplicateAndVerify/kotlin/gradle",files="test.versions.toml[tags=versions]"]
====

Add a small verification task (or a CI script) that parses both files and fails the build if the values diverge:

====
include::sample[dir="snippets/dependencyManagement/catalogs-duplicateAndVerify/kotlin",files="build.gradle.kts[tags=verify_shared_versions]"]
include::sample[dir="snippets/dependencyManagement/catalogs-duplicateAndVerify/groovy",files="build.gradle[tags=verify_shared_versions]"]
====
...
.gradle/shared-versions.properties
====
include::sample[dir="snippets/dependencyManagement/catalogs-externalProperties/kotlin/gradle",files="shared-versions.properties[tags=shared_versions]"]
====

====
include::sample[dir="snippets/dependencyManagement/catalogs-externalProperties/kotlin",files="settings.gradle.kts[tags=inject_versions]"]
include::sample[dir="snippets/dependencyManagement/catalogs-externalProperties/groovy",files="settings.gradle[tags=inject_versions]"]
====
...
.gradle/distribution.versions.toml
====
include::sample[dir="snippets/dependencyManagement/catalogs-externalProperties/kotlin/gradle",files="distribution.versions.toml[tags=catalog]"]
====
...
====
include::sample[dir="snippets/dependencyManagement/catalogs-publishedCatalogOverrides/kotlin/catalog",files="build.gradle.kts[tags=publish_catalog]"]
include::sample[dir="snippets/dependencyManagement/catalogs-publishedCatalogOverrides/groovy/catalog",files="build.gradle[tags=publish_catalog]"]
====

Consuming project (`settings.gradle(.kts)`):

====
include::sample[dir="snippets/dependencyManagement/catalogs-publishedCatalogOverrides/kotlin/consumer",files="settings.gradle.kts[tags=consume_with_overrides]"]
include::sample[dir="snippets/dependencyManagement/catalogs-publishedCatalogOverrides/groovy/consumer",files="settings.gradle[tags=consume_with_overrides]"]
====
...
dependencyResolutionManagement {
    versionCatalogs {
        create("libs") {
            // 1. Load the base catalog from a TOML file
            from(files("gradle/libs.versions.toml"))

            // 2. Override an existing version
            //    If "groovy" is already declared in the TOML file,
            //    this replaces its value.
            version("groovy", "3.0.6")

            // 3. Add a new version not present in the TOML file
            version("errorProne", "2.28.0")

            // 4. Add a library that uses the injected version
...
```kotlin
// These are equivalent in a dependencies {} block:
implementation(libs.groovy.core)
implementation(libs.groovy.core.get())
```

However, in other Gradle APIs — such as dependency substitution rules, dependency constraints, or any API that
accepts a dependency notation — **prefer passing the provider** (`libs.groovy.core`) rather than the unwrapped
value (`libs.groovy.core.get()`):

```kotlin
// Preferred — pass the provider:
...
Unwrapping with `.get()` gives you a `MinimalExternalModuleDependency`, which implements
`ExternalDependency` and `ModuleVersionSelector` — but some resolution APIs expect
`ModuleComponentSelector`, a different type.
Passing the provider avoids mismatches between these types.

**Note:** Version catalog entries carry dependency coordinates (group, name, and version constraint), but they do not carry classifier, artifact type, exclude, or capability information.
When you need any of these, apply them at the declaration site in your build script.
See <<sec:overriding-catalog-versions,Overwriting catalog versions>> for examples.
